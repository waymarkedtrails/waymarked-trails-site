...
class HikingStyleDefault(PGTable):
    ...
    def create(self):
        ...
        self.add_geometry_column("geom100", "3857", 'GEOMETRY', with_index=True)
...
class _SegmentInfo:
        ...
        if relinfo['network'] == 'CH':
            ...
            if relinfo['symbol'] is not None:
                if classification >= 0x4000 and len(self.inrshields) < 5:
                    self.inrshields.add(relinfo['symbol'])
                if len(self.allshields) < 5:
                    self.allshields.add(relinfo['symbol'])
    ...
    def make_db_fields(self, id=None):
        ...
        fields['inrshields'] = list(self.inrshields)
        fields['allshields'] = list(self.allshields)
