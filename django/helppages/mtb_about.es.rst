...
Este mapa muestra rutas señalizadas de MounTainBike (MTB) en todo el mundo. Se basa en los datos del proyecto `OpenStreetMap`_ (OSM). OSM es un mapa mundial que se puede modificar libremente donde cualquiera puede participar. Eso significa que este mapa para bicicletas no está completo, y también significa que usted puede contribuir mediante la adición de nuevas rutas y la corrección de errores en las existentes. Para obtener más información acerca de OpenStreetMap, consulte la `Guía del principiante`_.
...
.. _`Guía del principiante`: http://wiki.openstreetmap.org/wiki/ES:Beginners_Guide
.. _`Política de uso`: copyright

.. subpage:: rendering Representación de datos del OSM

Las rutas para MTB en el OSM deben ingresarse como relaciones. Cómo funciona esto se describe en detalle en la página de etiquetas (tags) de la wiki del OSM sobre `Rutas Ciclistas`_ (en inglés). Este mapa muestra todas las relaciones que tienen por lo menos las siguientes etiquetas: 
...
Nota: Las rutas para ciclismo no específico tienen `este mapa dedicado`_ y actualmente no son mostradas en este mapa. 
